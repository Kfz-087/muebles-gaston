<?php

require_once __DIR__ . '/../../config/conexion.php';
$conn = conectar();

$id = $_POST['id_cliente'];
$nombre = $_POST['nombre'];
$apellido = $_POST['apellido'];
$direccion = $_POST['direccion'];
$localidad = $_POST['localidad'];
$telefono = $_POST['telefono'];
$email = $_POST['email'];
$usuario = $_POST['usuario'];
$password = $_POST['password'];
$postal = $_POST['postal'];
$ruta = $_POST['ruta'];

// Handle image upload if a new file is provided
if (isset($_FILES['imagen']) && $_FILES['imagen']['error'] === UPLOAD_ERR_OK) {
    $nombre_imagen = $_FILES['imagen']['name'];
    // Absolute path on server to save the file
    $target_dir = __DIR__ . '/../../assets/profile/';

    // Create directory if it doesn't exist (safety check)
    if (!file_exists($target_dir)) {
        mkdir($target_dir, 0777, true);
    }

    $ruta_archivo_servidor = $target_dir . basename($nombre_imagen);

    // Move the uploaded file
    if (move_uploaded_file($_FILES['imagen']['tmp_name'], $ruta_archivo_servidor)) {
        // Path to store in DB (relative to web root or as expected by frontend)
        // Assuming frontend expects "/distribuidora-frami/assets/productos/..." or similar
        // Based on previous code: "/assets/productos/" . $nombre_imagen
        // But user code had: $ruta_imagen = "/assets/productos/" . $nombre_imagen;
        $ruta_db = "/distribuidora-frami/assets/profile/" . $nombre_imagen;

        $sql = "UPDATE clientes SET nombre=:nombre, apellido=:apellido, direccion=:direccion, localidad=:localidad, telefono=:telefono, correo=:correo, usuario=:usuario, contrasena=:contrasena, postal=:postal, imagen=:ruta WHERE id_cliente=:id";
        $params = [
            ':nombre' => $nombre,
            ':apellido' => $apellido,
            ':direccion' => $direccion,
            ':localidad' => $localidad,
            ':telefono' => $telefono,
            ':correo' => $email,
            ':usuario' => $usuario,
            ':contrasena' => $password,
            ':postal' => $postal,
            ':ruta' => $ruta_db,
            ':id' => $id
        ];
    } else {
        echo "Error al subir la imagen.";
        exit;
    }

} else {
    // Update without changing the image
    $sql = "UPDATE clientes SET nombre=:nombre, apellido=:apellido, direccion=:direccion, localidad=:localidad, telefono=:telefono, correo=:correo, usuario=:usuario, contrasena=:contrasena, postal=:postal, ruta=:ruta WHERE id_cliente=:id";
    $params = [
        ':nombre' => $nombre,
        ':apellido' => $apellido,
        ':direccion' => $direccion,
        ':localidad' => $localidad,
        ':telefono' => $telefono,
        ':correo' => $email,
        ':usuario' => $usuario,
        ':contrasena' => $password,
        ':postal' => $postal,
        ':id' => $id
    ];
}

$stmt = $conn->prepare($sql);

if ($stmt->execute($params)) {
    header("Location: ../index.php");
    exit();
} else {
    echo "Error al actualizar el cliente.";
}
?>